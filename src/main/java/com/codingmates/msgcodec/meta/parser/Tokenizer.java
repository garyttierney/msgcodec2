	...
	public Token next() {
		...
		if (Character.charCount(next) == 1) {
			...
			switch (character) {
				...
				case '@':
					return createToken(Token.Type.AT, character);
				case '(':
					return createToken(Token.Type.OPEN_PAREN, character);
				case ')':
					return createToken(Token.Type.CLOSE_PAREN, character);
				case '=':
					return createToken(Token.Type.EQUALS, character);
				case '"':
					return readString();
				case '#':
					return next();
